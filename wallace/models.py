...
class Node(Base):
    ...
    status = Column(Enum("alive", "dead", "failed", name="node_state"), nullable=False, default="alive")
    ...
    def connect_to(self, other_node):
        ...
        Will raise an error if:
            ...
            (2) other_node is a source or environment
        ...
        """
        from wallace.nodes import Source, Environment
        ...
        elif isinstance(other_node, Environment):
            raise(TypeError("{} cannot connect_to {} as it is an Environment.".format(self, other_node)))
    ...
    def transform(self, info_in, info_out=None, type=None):
        from wallace.transformations import check_for_transformation
        if info_out is None:
            raise ValueError("transform needs to be passed an info_out")
        if type is None:
            type = Transformation
        check_for_transformation(info_in=info_in, info_out=info_out)
        type(info_in=info_in, info_out=info_out)
...
class Vector(Base):
    ...
    status = Column(Enum("alive", "dead", "failed", name="vector_state"), nullable=False, default="alive")
...
class Transmission(Base):
    ...
    # the origin of the transmission, proxied from the vector
    origin_uuid = association_proxy('vector', 'origin_uuid')
    origin = association_proxy('vector', 'origin')
    ...
    network_uuid = association_proxy('vector', 'network_uuid')
    network = association_proxy('vector', 'network')
